(function () {
	...
	*/
	        ...
	        return clonedResponse.json().then(responseData => {
	          ...
	          switch (responseData.saveState) {
	            case SAVE_STATE_SERVER:
	              this._savedLocally = true; //this._savedRemotely = true;

	              this.savedRemotely = true;
	              break;

	            case SAVE_STATE_LOCAL:
	              this._savedLocally = true; //this._savedRemotely = false;
	          ...
	          this.modifiedStamp = parseInt(responseData.modified, 10); // return the json version of the original response as a promise
	...
	class Form extends EventHarness {
	  ...
	  get formElement() {
	    if (!this.#formEl) {
	      ...
	      this.buildContentContainer(this.#formEl); //this._formContentContainer = this.#formEl; // currently the form doesn't have any inner liner elements
	...
	class InternalAppError extends Error {} // MainController
	...
	class Page extends EventHarness {
	  ...
	  accordionItem(descriptor) {
	    ...
	    let buttonEl = headingEl.appendChild(document.createElement('button')); //buttonEl.className = `btn btn-link${(descriptor.collapsed ? ' collapsed' : '')}`;
	...
	class S {
	  ...
	  set_tetrad() {
	    if (this.tetradLetter = S.tetradLetters.substr(5 * (Math.floor(this.gridCoords.x % 1e4 / 1e3) >> 1) + (Math.floor(this.gridCoords.y % 1e4 / 1e3) >> 1), 1), !this.tetradLetter) throw new Error("Failed to get tetrad letter when processing '" + this.preciseGridRef + "', easting=" + this.gridCoords.x + " northing=" + this.gridCoords.y);
	    this.tetrad = this.hectad + this.tetradLetter;
	  }

	  static get_normalized_precision(S, N) {
	    return S > 2e3 ? 1e4 : S > 1e3 ? 2e3 : S > 100 ? 1e3 : S > 10 ? 100 : S > 1 ? 10 : N || 1;
	  }
	...
	class e {
	  ...
	  constructor(S, N) {
	    this.lat = S, this.lng = N;
	  }

	  static _transform(S, t, T, s, r, a, h, i, o, n, d, M, H, O) {
	    const l = 1e-6 * O;
	    let J = T / Math.sqrt(1 - s * (Math.sin(S) * Math.sin(S)));
	    const c = (J + r) * Math.cos(S) * Math.cos(t),
	          g = (J + r) * Math.cos(S) * Math.sin(t),
	          U = ((1 - s) * J + r) * Math.sin(S),
	          L = d / 3600 * N,
	          u = M / 3600 * N,
	          C = H / 3600 * N,
	          Y = c + c * l - g * C + U * u + i,
	          ...
	          P = -1 * c * u + g * L + U + U * l + n;
	    t = Math.atan(f / Y);
	    const D = Math.sqrt(Y * Y + f * f);
	    S = Math.atan(P / (D * (1 - h))), J = a / Math.sqrt(1 - h * (Math.sin(S) * Math.sin(S)));
	    let K = 1,
	        X = 0;

	    for (; K > .001;) X = Math.atan((P + h * J * Math.sin(S)) / D), K = Math.abs(X - S), S = X;

	    return new e(S, t);
	  }

	  static _Marc(S, N, t, e) {
	    return S * ((1 + N + 5 / 4 * (N * N) + 5 / 4 * (N * N * N)) * (e - t) - (3 * N + N * N * 3 + 21 / 8 * (N * N * N)) * Math.sin(e - t) * Math.cos(e + t) + (15 / 8 * (N * N) + 15 / 8 * (N * N * N)) * Math.sin(2 * (e - t)) * Math.cos(2 * (e + t)) - 35 / 24 * (N * N * N) * Math.sin(3 * (e - t)) * Math.cos(3 * (e + t)));
	  }
	...
	class s extends e {
	  ...
	  to_WGS84() {
	    ...
	    const s = this.lat * N,
	          r = Math.sin(s),
	          a = this.lng * N,
	          h = S / Math.sqrt(1 - e * (r * r)),
	          i = h * Math.cos(s) * Math.cos(a),
	          o = h * Math.cos(s) * Math.sin(a),
	          n = (1 - e) * h * r,
	          d = -204894e-10,
	          M = 7.28190110241429e-7,
	          H = 119748977294801e-20,
	          O = 446.448 + i * (1 + d) + -M * o + H * n,
	          l = 408261589226812e-20 * i - 124.157 + o * (1 + d) + -M * n,
	          J = 542.06 + -H * i + M * o + n * (1 + d);
	    S = 6378137, e = .00669438003;
	    const c = Math.sqrt(O * O + l * l);
	    let g = Math.atan(J / (c * (1 - e)));

	    for (let N = 1; N < 10; ++N) {
	      let N = Math.sin(g);
	      g = Math.atan((J + e * (S / Math.sqrt(1 - e * (N * N))) * N) / c);
	    }

	    return new T(t * g, t * Math.atan(l / O));
	  }
	  ...
	  static from_wgs84(S) {
	    const e = S.lat * N,
	          ...
	          a = .0066705397616,
	          h = 20.4894 * 1e-6;
	    let i = 6378137 / Math.sqrt(1 - r * Math.sin(e) * Math.sin(e));
	    const o = (i + 0) * Math.cos(e) * Math.cos(T),
	          n = (i + 0) * Math.cos(e) * Math.sin(T),
	          d = ((1 - r) * i + 0) * Math.sin(e),
	          M = -.1502 / 3600 * N,
	          H = -.247 / 3600 * N,
	          O = -.8421 / 3600 * N,
	          l = o + o * h - n * O + d * H - 446.448,
	          J = o * O + n + n * h - d * M + 125.157,
	          c = -1 * o * H + n * M + d + d * h + -542.06,
	          g = Math.atan(J / l),
	          U = Math.sqrt(l * l + J * J);
	    let L = Math.atan(c / (U * (1 - a)));
	    i = 6377563.396 / Math.sqrt(1 - a * (Math.sin(L) * Math.sin(L)));
	    let u = 1,
	        C = 0;

	    for (; u > .001;) C = Math.atan((c + a * i * Math.sin(L)) / U), u = Math.abs(C - L), L = C;

	    return new s(L * t, g * t);
	  }
	...
	class r extends e {
	  ...
	  static from_wgs84(S) {
	    const T = S.lat * N,
	          s = S.lng * N,
	          a = e._transform(T, s, 6378137, .00669438037928458, 0, 6378388, .0067226700223333, 83.901, 98.127, 118.635, 0, 0, 0, 0);

	    return new r(a.lat * t, a.lng * t);
	  }
	...
	class a extends e {
	  ...
	  to_WGS84() {
	    const S = e._transform(this.lat * N, this.lng * N, 6377340.189, .00667054015, 0, 6378137, .00669438037928458, 482.53, -130.596, 564.557, -1.042, -.214, -.631, -8.15);

	    return new T(S.lat * t, S.lng * t);
	  }

	  static from_wgs84(S) {
	    const T = S.lat * N,
	          s = S.lng * N,
	          r = e._transform(T, s, 6378137, .00669438037928458, 0, 6377340.189, .00667054015, -482.53, 130.596, -564.557, 1.042, .214, .631, 8.15);

	    return new a(r.lat * t, r.lng * t);
	  }
	...
	class h {
	  ...
	  static from_latlng(S, N) {
	    if (N >= -8.74 && S > 49.88) {
	      let t = new s.from_wgs84(new T(S, N)).to_os_coords();
	      if (t.x >= 0 && t.is_gb_hectad()) return t;
	    }

	    if (N < -5.3 && S > 51.34 && N > -11 && S < 55.73) {
	      let t = new a.from_wgs84(new T(S, N)).to_os_coords();
	      return t.x < 0 || t.y < 0 ? null : t;
	    }

	    {
	      let t = new r.from_wgs84(new T(S, N)).to_os_coords();
	      if (t.x >= 5e5 && t.x < 6e5 && t.y >= 54e5 && t.y < 56e5) return t;
	    }
	    return null;
	  }

	  static calculate_tetrad(S, N) {
	    return S >= 0 && N >= 0 ? h.tetradLetters.charAt(5 * Math.floor(S % 1e4 / 2e3) + Math.floor(N % 1e4 / 2e3)) : "";
	  }
	...
	const i = function (S, N, t, e) {
	  let T = "00000" + Math.floor(N),
	      s = "00000" + Math.floor(t);
	  if (2e3 === e) return S + T.charAt(T.length - 5) + s.charAt(s.length - 5) + h.calculate_tetrad(N, t);
	  if (1e5 === e) return S;
	  {
	    5e3 === e && (e = 1e4);
	    let N = Math.round(Math.log10(e));
	    return S + (N ? T.slice(-5, -N) + s.slice(-5, -N) : T.slice(-5) + s.slice(-5));
	  }
	};
	...
	class o extends h {
	  ...
	  to_latLng() {
	    var S = .9996,
	        ...
	        s = 6356911.946 * S,
	        r = this.x - 5e5,
	        a = d(this.y, 0, e, 0, .0016863406508729017, s),
	        h = e / Math.sqrt(1 - N * (Math.sin(a) * Math.sin(a))),
	        i = h * (1 - N) / (1 - N * Math.sin(a) * Math.sin(a)),
	        o = h / i - 1,
	        M = Math.tan(a) * Math.tan(a),
	        H = Math.pow(Math.tan(a), 4),
	        O = Math.pow(Math.tan(a), 6),
	        l = Math.pow(Math.cos(a), -1),
	        J = Math.tan(a) / (2 * i * h),
	        c = Math.tan(a) / (24 * i * (h * h * h)) * (5 + 3 * M + o - 9 * o * M),
	        g = Math.tan(a) / (720 * i * Math.pow(h, 5)) * (61 + 90 * M + 45 * H),
	        U = a - r * r * J + Math.pow(r, 4) * c - Math.pow(r, 6) * g,
	        L = Math.pow(Math.cos(a), -1) / h,
	        u = l / (h * h * h * 6) * (h / i + 2 * M),
	        C = l / (120 * Math.pow(h, 5)) * (5 + 28 * M + 24 * H),
	        Y = l / (5040 * Math.pow(h, 7)) * (61 + 662 * M + 1320 * H + 720 * O),
	        f = r * L - .0523598775598 - r * r * r * u + Math.pow(r, 5) * C - Math.pow(r, 7) * Y,
	        P = n(U, f);
	    return new T(P.lat * t, P.lng * t);
	  }

	  to_gridref(S) {
	    return this.y >= 55e5 ? i("WA", this.x - 5e5, this.y - 55e5, S || 1) : this.y < 55e5 ? i("WV", this.x - 5e5, this.y - 54e5, S || 1) : null;
	  }

	  to_hectad() {
	    return this.y > 55e5 ? "WA" + this.x.toString().substring(1, 2) + this.y.toString().substring(2, 3) : this.y < 55e5 ? "WV" + this.x.toString().substring(1, 2) + this.y.toString().substring(2, 3) : null;
	  }

	}

	const n = function (S, N) {
	  return e._transform(S, N, 6378388, .0067226700223333, 10, 6378137, .00669438037928458, -83.901, -98.127, -118.635, 0, 0, 0, 0);
	},
	      d = function (S, N, t, T, s, r) {
	  for (var a = (S - N) / t + T, h = e._Marc(r, s, T, a), i = (S - N - h) / t + a, o = 0; Math.abs(S - N - h) > 1e-5 && o < 20;) o += 1, i = (S - N - h) / t + a, h = e._Marc(r, s, T, i), a = i;
	...
	class M extends S {
	  ...
	  static gridref_string_to_e_n_l(S) {
	    let N,
	        ...
	        s = S.substr(0, 2);
	    if ("WA" === s) N = 55e5;else {
	      if ("WV" !== s) return console.log("Bad Channel Island grid letters: '" + s + "'"), !1;
	      N = 54e5;
	    }
	    let r = S.substr(2);
	    ...
	    switch (r.length) {
	      case 2:
	        t = 1e4 * r.charAt(0), e = 1e4 * r.charAt(1), T = 1e4;
	        break;

	      case 4:
	        t = 1e3 * r.substr(0, 2), e = 1e3 * r.substr(2), T = 1e3;
	        break;

	      case 6:
	        t = 100 * r.substr(0, 3), e = 100 * r.substr(3), T = 100;
	        break;

	      case 8:
	        t = 10 * r.substr(0, 4), e = 10 * r.substr(4), T = 10;
	        break;

	      case 10:
	        t = parseInt(r.substr(0, 5), 10), e = parseInt(r.substr(5), 10), T = 1;
	        break;

	      default:
	        return console.log("Bad length for Channel Island grid ref '" + S + "'"), !1;
	    }
	...
	class H extends h {
	  ...
	  to_gridref(S) {
	    const N = this.x / 1e5 | 0,
	          t = this.y / 1e5 | 0;
	    let e;
	    e = t < 5 ? N < 5 ? "S" : "T" : t < 10 ? N < 5 ? "N" : "O" : N < 5 ? "H" : "J";
	    let T = 65 + 5 * (4 - t % 5) + N % 5;
	    T >= 73 && T++;
	    const s = String.fromCharCode(T);
	    return i(e + s, this.x - 1e5 * N, this.y - 1e5 * t, S || 1);
	  }

	  to_hectad() {
	    const S = this.x / 1e5 | 0,
	          N = this.y / 1e5 | 0;
	    let t;
	    t = N < 5 ? S < 5 ? "S" : "T" : N < 10 ? S < 5 ? "N" : "O" : S < 5 ? "H" : "J";
	    let e = 65 + 5 * (4 - N % 5) + S % 5;
	    return e >= 73 && e++, t + String.fromCharCode(e) + ((this.x - 1e5 * S) / 1e4 | 0) + ((this.y - 1e5 * N) / 1e4 | 0);
	  }

	  is_gb_hectad() {
	    return -1 !== H.gbHectads.indexOf(this.to_hectad());
	  }
	  ...
	  to_latLng() {
	    const S = 4e5,
	          ...
	          h = .0016732203289875;
	    let i,
	        o = (a + 1e5) / (.9996012717 * e) + N;

	    do {
	      i = a + 1e5 - 6353722.489 * (1.0016767257674 * (o - N) - .00502807228247412 * Math.sin(o - N) * Math.cos(o + N) + (1.875 * h * h + 1.875 * h * h * h) * Math.sin(2 * (o - N)) * Math.cos(2 * (o + N)) - 35 / 24 * h * h * h * Math.sin(3 * (o - N)) * Math.cos(3 * (o + N))), o += i / 6375020.48098897;
	    } while (i >= .001);

	    const n = Math.sin(o) * Math.sin(o),
	          d = Math.tan(o) * Math.tan(o),
	          M = 1 / Math.cos(o),
	          H = .9996012717 * e * Math.pow(1 - T * n, -.5),
	          O = 6332495.651423464 * Math.pow(1 - T * n, -1.5),
	          l = H / O - 1,
	          J = Math.tan(o) / (2 * O * H),
	          c = Math.tan(o) / (24 * O * Math.pow(H, 3)) * (5 + 3 * d + l - 9 * d * l),
	          g = Math.tan(o) / (720 * O * Math.pow(H, 5)) * (61 + 90 * d + 45 * d * d),
	          U = M / H,
	          L = M / (6 * H * H * H) * (H / O + 2 * d),
	          u = M / (120 * Math.pow(H, 5)) * (5 + 28 * d + 24 * d * d),
	          C = M / (5040 * Math.pow(H, 7)) * (61 + 662 * d + 1320 * d * d + 720 * d * d * d),
	          Y = o - J * Math.pow(r - S, 2) + c * Math.pow(r - S, 4) - g * Math.pow(r - S, 6),
	          f = U * (r - S) - .034906585039887 - L * Math.pow(r - S, 3) + u * Math.pow(r - S, 5) - C * Math.pow(r - S, 7);
	    return new s(t * Y, t * f).to_WGS84();
	  }
	...
	class O extends S {
	  ...
	  parse_well_formed(N) {
	    N.length >= 5 && /^[A-Z]/.test(N.charAt(4)) && (S.quadrantOffsets.hasOwnProperty(N.substr(N.length - 2)) ? this.quadrantCode = N.substr(N.length - 2) : this.tetradLetter = N.charAt(4), N = N.substr(0, 4)), this.parse_wellformed_gb_gr_string_no_tetrads(N), this.tetradLetter || this.quadrantCode ? this.tetradLetter ? (this.preciseGridRef = this.tetrad = this.hectad + this.tetradLetter, this.length = 2e3, this.gridCoords.x += S.tetradOffsets[this.tetradLetter][0], this.gridCoords.y += S.tetradOffsets[this.tetradLetter][1]) : (this.preciseGridRef = this.quadrant = N + this.quadrantCode, this.length = 5e3, this.gridCoords.x += S.quadrantOffsets[this.quadrantCode][0], this.gridCoords.y += S.quadrantOffsets[this.quadrantCode][1]) : (this.preciseGridRef = N, this.length <= 1e3 && this.set_tetrad());
	  }
	  ...
	  from_string(N) {
	    let t,
	        e = N.replace(/[\[\]\s\t.-]+/g, "").toUpperCase(),
	        T = "";
	    if (/[ABCDEFGHIJKLMNPQRSTUVWXYZ]$/.test(e) && (S.quadrantOffsets.hasOwnProperty(e.substr(e.length - 2)) ? (this.quadrantCode = e.substr(e.length - 2), e = e.substr(0, e.length - 2)) : (T = e.substr(e.length - 1), e = e.substr(0, e.length - 1))), e === parseInt(e, 10).toString() ? e = e.substr(0, 2) + "/" + e.substr(2) : e.length > 3 && "/" === e.charAt(2) && /^[A-Z]{2}$/.test(e.substr(0, 2)) && (e = e.replace("/", "")), "VC" === e.substr(0, 2)) this.error = !0, this.errorMessage = "Misplaced vice-county code in grid-reference field. ('" + e + "')", this.gridCoords = null, this.length = 0;else if (null !== (t = e.match(/^([HJNOST][ABCDEFGHJKLMNOPQRSTUVWXYZ](?:\d\d){1,5})$/))) e = t[0], this.parse_wellformed_gb_gr_string_no_tetrads(e), this.length > 0 ? 1e4 === this.length && (T || this.quadrantCode) ? T ? (this.preciseGridRef = e + T, this.tetradLetter = T, this.tetrad = this.hectad + T, this.length = 2e3, this.gridCoords.x += S.tetradOffsets[T][0], this.gridCoords.y += S.tetradOffsets[T][1]) : (this.preciseGridRef = e + this.quadrantCode, this.tetradLetter = "", this.tetrad = "", this.quadrant = this.preciseGridRef, this.length = 5e3, this.gridCoords.x += S.quadrantOffsets[this.quadrantCode][0], this.gridCoords.y += S.quadrantOffsets[this.quadrantCode][1]) : (this.preciseGridRef = e, this.length <= 1e3 && this.set_tetrad()) : (this.error = !0, this.errorMessage = "GB grid reference format not understood (strange length).");else if (/^([\d]{2})\/((?:\d\d){1,5})$/.test(e)) {
	      switch (this.parse_gr_string_without_tetrads(e), this.length) {
	        case 1e4:
	          e = this.gridCoords.to_gridref(1e4), this.hectad = e, T ? (e += T, this.tetradLetter = T, this.tetrad = this.hectad + T, this.length = 2e3, this.gridCoords.x += S.tetradOffsets[T][0], this.gridCoords.y += S.tetradOffsets[T][1]) : this.quadrantCode && (e += this.quadrantCode, this.quadrant = e, this.length = 5e3, this.gridCoords.x += S.quadrantOffsets[this.quadrantCode][0], this.gridCoords.y += S.quadrantOffsets[this.quadrantCode][1]);
	          break;
	        ...
	        case 10:
	        case 1:
	          e = this.gridCoords.to_gridref(this.length), this.hectad = this.gridCoords.to_gridref(1e4), this.set_tetrad();
	          break;

	        default:
	          this.error = !0, this.errorMessage = "Bad grid square dimension (" + this.length + " m).", this.gridCoords = null, this.length = 0;
	      }

	      this.preciseGridRef = e;
	    } else this.gridCoords = null, this.length = 0, this.error = !0, this.errorMessage = "Grid reference format not understood. ('" + N + "')";
	  }

	  parse_gr_string_without_tetrads(N) {
	    let t, e, T, s;

	    if (null !== (t = N.match(/^(\d{2})\/((?:\d\d){1,5})$/))) {
	      switch (t[1]) {
	        case "57":
	          e = 3e5, T = 1e6;
	          break;
	        ...
	        default:
	          e = 1e5 * N.charAt(0), T = 1e5 * N.charAt(1);
	      }

	      s = t[2];
	    } else {
	      if (!S.letterMapping.hasOwnProperty(N.charAt(0)) || !S.letterMapping.hasOwnProperty(N.charAt(1))) return this.length = 0, void (this.gridCoords = null);
	      let t = S.letterMapping[N.charAt(0)],
	          r = S.letterMapping[N.charAt(1)];
	      s = N.substr(2), e = t % 5 * 5e5 + r % 5 * 1e5 - 1e6, T = 5e5 * -Math.floor(t / 5) - 1e5 * Math.floor(r / 5) + 19e5;
	    }

	    switch (s.length) {
	      case 2:
	        this.gridCoords = new H(e + 1e4 * s.charAt(0), T + 1e4 * s.charAt(1)), this.length = 1e4;
	        break;

	      case 4:
	        this.gridCoords = new H(e + 1e3 * Math.floor(s / 100), T + s % 100 * 1e3), this.length = 1e3;
	        break;

	      case 6:
	        this.gridCoords = new H(e + 100 * Math.floor(s / 1e3), T + s % 1e3 * 100), this.length = 100;
	        break;

	      case 8:
	        this.gridCoords = new H(e + 10 * Math.floor(s / 1e4), T + s % 1e4 * 10), this.length = 10;
	        break;

	      case 10:
	        this.gridCoords = new H(e + Math.floor(s / 1e5), T + s % 1e5), this.length = 1;
	        break;

	      default:
	        console.log("Bad grid ref length, ref=" + N), this.gridCoords = null, this.length = 0;
	    }
	  }

	  parse_wellformed_gb_gr_string_no_tetrads(N) {
	    let t, e, T, s, r;

	    switch (t = S.letterMapping[N.charAt(0)], e = S.letterMapping[N.charAt(1)], T = N.substr(2), s = t % 5 * 5e5 + e % 5 * 1e5 - 1e6, r = 5e5 * -Math.floor(t / 5) - 1e5 * Math.floor(e / 5) + 19e5, T.length) {
	      case 2:
	        this.gridCoords = new H(s + 1e4 * T.charAt(0), r + 1e4 * T.charAt(1)), this.length = 1e4, this.hectad = N;
	        break;

	      case 4:
	        this.gridCoords = new H(s + 1e3 * Math.floor(T / 100), r + T % 100 * 1e3), this.length = 1e3, this.hectad = N.substr(0, 3) + N.substr(4, 1);
	        break;

	      case 6:
	        this.gridCoords = new H(s + 100 * Math.floor(T / 1e3), r + T % 1e3 * 100), this.length = 100, this.hectad = N.substr(0, 3) + N.substr(5, 1);
	        break;

	      case 8:
	        this.gridCoords = new H(s + 10 * Math.floor(T / 1e4), r + T % 1e4 * 10), this.length = 10, this.hectad = N.substr(0, 3) + N.substr(6, 1);
	        break;

	      case 10:
	        this.gridCoords = new H(s + Math.floor(T / 1e5), r + T % 1e5), this.length = 1, this.hectad = N.substr(0, 3) + N.substr(7, 1);
	        break;

	      default:
	        throw this.gridCoords = null, new Error("Bad grid ref length when parsing supposedly well-formed ref, ref='" + N + "'");
	    }
	  }
	...
	class l extends h {
	  ...
	  to_latLng() {
	    const S = 1.000035,
	          ...
	          s = this.x - 2e5,
	          r = .0067153352074207,
	          h = (5929615.3530033 + (this.y - 25e4) / S) / 6366691.7742864415,
	          i = h + .002509826623715886 * Math.sin(2 * h) + 36745487490091978e-22 * Math.sin(4 * h) + 151 * T * T * T / 96 * Math.sin(6 * h),
	          o = N / Math.sqrt(1 - e * Math.sin(i) * Math.sin(i)),
	          n = Math.tan(i) * Math.tan(i),
	          d = r * Math.cos(i) * Math.cos(i),
	          M = N * (1 - e) / Math.pow(1 - e * Math.sin(i) * Math.sin(i), 1.5),
	          H = s / (o * S);
	    let O = i - o * Math.tan(i) / M * (H * H / 2 - (5 + 3 * n + 10 * d - 4 * d * d - 9 * r) * H * H * H * H / 24 + (61 + 90 * n + 298 * d + 45 * n * n - 1.6922644722700164 - 3 * d * d) * H * H * H * H * H * H / 720);
	    O *= t;
	    let l = (H - (1 + 2 * n + d) * H * H * H / 6 + (5 - 2 * d + 28 * n - 3 * d * d + 8 * r + 24 * n * n) * H * H * H * H * H / 120) / Math.cos(i);
	    return l = l * t - 8, new a(O, l).to_WGS84();
	  }

	  to_gridref(S) {
	    const N = Math.floor(this.x / 1e5),
	          t = Math.floor(this.y / 1e5);
	    return l.irishGrid[N] && l.irishGrid[N][t] ? i(l.irishGrid[N][t], this.x - 1e5 * N, this.y - 1e5 * t, S || 1) : null;
	  }

	  to_hectad() {
	    const S = Math.floor(this.x / 1e5),
	          N = Math.floor(this.y / 1e5);
	    return l.irishGrid[S] && l.irishGrid[S][N] ? l.irishGrid[S][N] + Math.floor(this.x % 1e5 / 1e4) + Math.floor(this.y % 1e5 / 1e4) : "";
	  }
	...
	class J extends S {
	  ...
	  from_string(S) {
	    let N = S.replace(/[\[\]\s\t.-]+/g, "").toUpperCase();
	    /[ABCDEFGHIJKLMNPQRSTUVWXYZ]$/.test(N) && (J.quadrantOffsets.hasOwnProperty(N.substr(N.length - 2)) ? (this.quadrantCode = N.substr(N.length - 2), N = N.substr(0, N.length - 2)) : (this.tetradLetter = N.substr(N.length - 1), N = N.substr(0, N.length - 1))), this.parse_gr_string_without_tetrads(N), this.length > 0 ? this.tetradLetter || this.quadrantCode ? this.tetradLetter ? (this.preciseGridRef = this.hectad + this.tetradLetter, this.tetrad = this.preciseGridRef, this.length = 2e3, this.gridCoords.x += J.tetradOffsets[this.tetradLetter][0], this.gridCoords.y += J.tetradOffsets[this.tetradLetter][1]) : (this.preciseGridRef = this.hectad + this.quadrantCode, this.quadrant = this.preciseGridRef, this.length = 5e3, this.gridCoords.x += J.quadrantOffsets[this.quadrantCode][0], this.gridCoords.y += J.quadrantOffsets[this.quadrantCode][1]) : (this.preciseGridRef = N, this.length <= 1e3 && this.set_tetrad()) : (this.error = !0, this.errorMessage = "Irish grid reference format not understood. ('" + S + "')");
	  }

	  static _IE_GRID_LETTERS = "VQLFAWRMGBXSNHCYTOJD";

	  parse_gr_string_without_tetrads(S) {
	    let N, t, e, T;

	    if (/^\d{2}\/(?:\d\d){1,5}$/.test(S)) {
	      if (N = parseInt(S.charAt(0), 10), t = parseInt(S.charAt(1), 10), N > 3 || t > 4) return console.log("bad grid square, ref='" + S + "' (Ireland)"), this.length = 0, !1;
	      e = S.substr(3), T = J._IE_GRID_LETTERS.charAt(5 * N + t), N *= 1e5, t *= 1e5;
	    } else {
	      if (S = S.replace("/", ""), !/^[ABCDFGHJLMNOQRSTVWXY](?:\d\d){1,5}$/.test(S)) return this.length = 0, this.gridCoords = null, !1;
	      if (!S) return console.log("Bad (empty) Irish grid ref"), this.length = 0, this.gridCoords = null, !1;
	      {
	        T = S.charAt(0);

	        let e = J._IE_GRID_LETTERS.indexOf(T);

	        if (-1 === e) return console.log("Bad grid ref grid-letter, ref='" + S + "' (Ireland)"), this.length = 0, this.gridCoords = null, !1;
	        N = 1e5 * Math.floor(e / 5), t = e % 5 * 1e5;
	      }
	      e = S.substr(1);
	    }
	    ...
	    switch (e.length) {
	      case 2:
	        this.gridCoords = new l(N + 1e4 * e.charAt(0), t + 1e4 * e.charAt(1)), this.length = 1e4, this.hectad = T + e;
	        break;

	      case 4:
	        this.gridCoords = new l(N + 1e3 * Math.floor(e / 100), t + e % 100 * 1e3), this.length = 1e3, this.hectad = T + e.charAt(0) + e.charAt(2);
	        break;

	      case 6:
	        this.gridCoords = new l(N + 100 * Math.floor(e / 1e3), t + e % 1e3 * 100), this.length = 100, this.hectad = T + e.charAt(0) + e.charAt(3);
	        break;

	      case 8:
	        this.gridCoords = new l(N + 10 * Math.floor(e / 1e4), t + e % 1e4 * 10), this.length = 10, this.hectad = T + e.charAt(0) + e.charAt(4);
	        break;

	      case 10:
	        this.gridCoords = new l(N + Math.floor(e / 1e5), t + e % 1e5), this.length = 1, this.hectad = T + e.charAt(0) + e.charAt(5);
	        break;

	      default:
	        return console.log("Bad grid ref length, ref='" + S + "' (Ireland)"), this.length = 0, this.gridCoords = null, !1;
	    }
	...
	S.from_string = function (S) {
	  let N,
	      t = S.replace(/\s+/g, "").toUpperCase();
	  if (!t) return !1;
	  if (/^(?:[BCDFGHJLMNOQRSTVWXY]|[HJNOST][ABCDEFGHJKLMNOPQRSTUVWXYZ]|W[VA])\d{2}(?:[A-Z]|[NS][EW]|(?:\d{2}){0,4})?$/.test(t)) return N = /^.\d/.test(t) ? new J() : "W" === t.charAt(0) ? new M() : new O(), N.parse_well_formed(t), !(!N.length || N.error) && N;
	  if (N = new O(), N.from_string(t), N.length && !N.error) return N;

	  if ("W" === t.charAt(0)) {
	    if (N = new M(), N.from_string(t), N.length && !N.error) return N;
	  } else if (N = new J(), N.from_string(t), N.length && !N.error) return N;

	  return !1;
	};

	const c = H;
	      (s.prototype.to_os_coords = function () {
	  var S = this.lat * N,
	      t = this.lng * N,
	      T = .9996012717,
	      s = .0066705397616,
	      r = 6377563.396 * T,
	      a = 6356256.91 * T,
	      h = Math.sin(S) * Math.sin(S),
	      i = r / Math.sqrt(1 - s * h),
	      o = i * (1 - s) / (1 - s * h),
	      n = i / o - 1,
	      d = t - -.03490658503988659,
	      M = i * Math.cos(S),
	      H = Math.pow(Math.cos(S), 3),
	      O = Math.tan(S) * Math.tan(S),
	      l = i / 6 * H * (i / o - O),
	      J = Math.pow(Math.cos(S), 5),
	      g = Math.pow(Math.tan(S), 4),
	      U = i / 120 * J * (5 - 18 * O + g + 14 * n - 58 * O * n),
	      L = 4e5 + d * M + Math.pow(d, 3) * l + Math.pow(d, 5) * U,
	      u = e._Marc(a, .0016732202503250907, .8552113334772214, S) + -1e5,
	      C = i / 2 * Math.sin(S) * Math.cos(S),
	      Y = i / 24 * Math.sin(S) * Math.pow(Math.cos(S), 3) * (5 - Math.pow(Math.tan(S), 2) + 9 * n),
	      f = i / 720 * Math.sin(S) * J * (61 - 58 * O + g),
	      P = u + d * d * C + Math.pow(d, 4) * Y + Math.pow(d, 6) * f;
	  return new c(Math.round(L), Math.round(P));
	}, s);
	      const U = l;
	      (a.prototype.to_os_coords = function () {
	  var S = this.lat * N,
	      t = this.lng * N,
	      T = 1.000035,
	      s = .00667054015,
	      ...
	      o = i * (1 - s) / (1 - s * h),
	      n = i / o - 1,
	      d = t - -.13962634015954636,
	      M = i * Math.cos(S),
	      H = Math.pow(Math.cos(S), 3),
	      O = Math.tan(S) * Math.tan(S),
	      l = i / 6 * H * (i / o - O),
	      J = Math.pow(Math.cos(S), 5),
	      c = Math.pow(Math.tan(S), 4),
	      g = i / 120 * J * (5 - 18 * O + c + 14 * n - 58 * O * n),
	      L = 2e5 + d * M + Math.pow(d, 3) * l + Math.pow(d, 5) * g,
	      u = e._Marc(a, .0016732203841520518, .9337511498169663, S) + 25e4,
	      C = i / 2 * Math.sin(S) * Math.cos(S),
	      Y = i / 24 * Math.sin(S) * Math.pow(Math.cos(S), 3) * (5 - Math.pow(Math.tan(S), 2) + 9 * n),
	      f = i / 720 * Math.sin(S) * J * (61 - 58 * O + c),
	      P = u + d * d * C + Math.pow(d, 4) * Y + Math.pow(d, 6) * f;
	  return new U(Math.round(L), Math.round(P));
	}, a);
	      const u = o;
	      (r.prototype.to_os_coords = function () {
	  var S = this.lat * N,
	      t = this.lng * N,
	      T = .9996,
	      s = .0067226700223333,
	      ...
	      c = Math.pow(Math.tan(S), 4),
	      g = i / 120 * J * (5 - 18 * O + c + 14 * n - 58 * O * n),
	      U = 5e5 + d * M + Math.pow(d, 3) * l + Math.pow(d, 5) * g,
	      L = e._Marc(a, .0016863406508729017, 0, S) + 0,
	      C = i / 2 * Math.sin(S) * Math.cos(S),
	      Y = i / 24 * Math.sin(S) * Math.pow(Math.cos(S), 3) * (5 - Math.pow(Math.tan(S), 2) + 9 * n),
	      f = i / 720 * Math.sin(S) * J * (61 - 58 * O + c),
	      P = L + d * d * C + Math.pow(d, 4) * Y + Math.pow(d, 6) * f;
	  return new u(Math.round(U), Math.round(P));
	}, r);
	/**
	...
	class GPSRequest extends EventHarness {
	  ...
	  static seekGPS(gpsPromptBannerId) {
	    ...
	    return new Promise((resolve, reject) => navigator.geolocation.getCurrentPosition(resolve, reject, {
	      ...
	      const latitude = position.coords.latitude;
	      const longitude = position.coords.longitude;
	      console.log(`Got GPS fix ${latitude} , ${longitude}`); //@todo maybe should prevent use of readings if speed is too great (which might imply use of GPS in a moving vehicle)
	...
	class TextGeorefField extends FormField {
	  ...
	  seekGPS() {
	    if (this._seekingGPS) {
	      ...
	      return GPSRequest.seekGPS(this._gpsPermissionsPromptId).then(position => {
	        this._seekingGPS = false; // const latitude  = position.coords.latitude;
	        // const longitude = position.coords.longitude;
	        // console.log(`Got GPS fix ${latitude} , ${longitude}`);
	        //
	        // const gridCoords = GridCoords.from_latlng(latitude, longitude);
	        ...
	        this.processLatLngPosition(position.coords.latitude, position.coords.longitude, accuracy, TextGeorefField.GEOREF_SOURCE_GPS);
	      }, error => {
	  ...
	  processLatLngPosition(latitude, longitude, precision, source, rawString = '') {
	    ...
	    console.log(`Got grid-ref: ${gridRef}`); //this.value = gridRef;
	  ...
	  static summariseImpl(key, property, attributes) {
	    return attributes[key] !== '' && attributes[key] !== null && attributes[key] !== undefined && attributes[key].gridRef ? `<span>grid-reference <span class="gridref-summary">${escapeHTML(attributes[key].gridRef.trim())}</span></span>` : '';
	  }
	  /**
	   *
	   * @param value
	...
	class App extends EventHarness {
	  ...
	  initialise() {
	    ...
	    this.#router.notFound(query => {
	      ...
	      console.log(`no route found for '${query}'`); //this.#router.navigate('/list');
	    ...
	    this.#router.on(() => {
	      ...
	      this.#router.pause(); //if (this.clearCurrentSurvey && this.currentSurvey.isPristine) { // this appears to be a bug 'this.clearCurrentSurvey'
	  ...
	  display() {
	    ...
	    this.#router.resolve(); // it's opportune at this point to try to ping the server again to save anything left outstanding
	...
	class BSBIServiceWorker {
	  ...
	  initialise(configuration) {
	    ...
	    self.addEventListener('install', evt => {
	      ...
	      evt.waitUntil(this.precache() // see https://serviceworke.rs/immediate-claim_service-worker_doc.html
	      // .finally(() => {
	    ...
	    evt => {
	      ...
	      if (evt.request.method === 'POST') {
	        ...
	        if (SERVICE_WORKER_INTERCEPT_URL_MATCHES.test(evt.request.url) && !SERVICE_WORKER_IGNORE_URL_MATCHES.test(evt.request.url)) {
	          ...
	          console.log(`request is for non-image '${evt.request.url}'`); // You can use `respondWith()` to answer immediately, without waiting for the
	          // network response to reach the service worker...

	          evt.respondWith(this.fromCache(evt.request)); // ...and `waitUntil()` to prevent the worker from being killed until the
	          // cache is updated.
	  ...
	  handle_post(evt) {
	    ...
	    evt.respondWith(fetch(evt.request).then(response => {
	      ...
	      console.log(`post fetch failed (probably no network), (reason: ${reason})`); //console.log({'post failure reason' : reason});
	      // /**
	  ...
	  fromCache(request) {
	    ...
	    return caches.open(this.CACHE_VERSION).then(cache => {
	      ...
	      return cache.match(request).then(matching => {
	        console.log(matching ? `cache matched ${request.url}` : `no cache match for ${request.url}`); //return matching || fetch(request); // return cache match or if not cached then go out to network
	...
	serviceWorker.initialise({
	  ...
	  urlCacheSet: ['./index.html', './app.js?version=1.0.3.1644336565', './manifest.webmanifest', '/appcss/app.2021-12-16.css', // note no leading '.' - this is an absolute path
